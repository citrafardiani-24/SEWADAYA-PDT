...
<div class="bg-white p-6 rounded-lg shadow-md mb-8">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <?php
        try {
            // Ambil semua item pakaian yang stoknya lebih dari 0 dari database
            $stmt = $pdo->query("SELECT * FROM clothing_items WHERE stock > 0 ORDER BY name ASC");
            $items = $stmt->fetchAll(PDO::FETCH_ASSOC);

            if (count($items) > 0) {
                // Iterasi setiap item untuk ditampilkan
                foreach ($items as $item): ?>
                    <div class="border border-gray-200 rounded-lg p-4 shadow-sm flex flex-col item-card">
                        <!-- Menampilkan gambar item dari jalur relatif yang disimpan di database -->
                        <!-- FIX: Corrected alt attribute syntax -->
                        ...
                        <div class="flex justify-between items-center text-sm text-gray-700 mb-2">
                            ...
                            <!-- Kolom Warna dihapus dari tampilan -->
                        </div>
                        ...
                        <?php if ($item['stock'] > 0): ?>
                            <!-- Tombol untuk menyewa item, akan memicu modal penyewaan -->
                            <!-- Mengirimkan data stok yang tersedia ke fungsi JS -->
            ...
            // Tangani error jika gagal memuat item dari database
